...
It also uses pretty straightforward systems to update these components, as you can see in \doxylink{systems_8hpp}{systems.\+hpp}\hypertarget{engine_autotoc_md135}{}\doxysubsection{\texorpdfstring{}{}}\label{engine_autotoc_md135}
...
\end{DoxyItemize}\hypertarget{engine_autotoc_md136}{}\doxysubsubsection{\texorpdfstring{1. Initialize the Engine}{1. Initialize the Engine}}\label{engine_autotoc_md136}
...
Then create a registry instance and a rendering window (for example using SFML)\+: 
\begin{DoxyCode}{0}
...
\end{DoxyCode}
 You just created a registry with a window. The engine then knows it needs to activate the rendering systems and its affiliate components. Don\textquotesingle{}t worry, basic components and systems are allready registered by default. Now, you can start creating an entity\+:
...
You just created an entity — a simple ID that will hold various components. Now let\textquotesingle{}s attach these components one by one to make it an actual {\bfseries{player}}.

\DoxyHorRuler{0}
 \hypertarget{engine_autotoc_md138}{}\doxysubsubsection{\texorpdfstring{2. Add the Components}{2. Add the Components}}\label{engine_autotoc_md138}
...
At first, its velocity is zero — it will only move once we apply input through the control system. We keep a reference to this newly created component because we might need to make changes on it on the future, to handle some events.
...
“\+This entity can be controlled by the player.\+” When the engine will detect input, the comtrollable component will be updated. That’s why we keep a reference to this newly created component. We will later need to check its value to make the player move \end{quote}
...
The {\ttfamily set\+Texture\+From\+Name("{}player1"{})} is a custom helper that loads a texture previously registered in your asset manager. To register a texture in the resource manager, see \doxylink{ResourceManager_8hpp}{Resource\+Manager.\+hpp}. {\ttfamily Resource\+Manager\+::instance().load("{}path/to/your/texture.\+png"{}, TEXTURE);} \hypertarget{engine_autotoc_md142}{}\doxysubsection{\texorpdfstring{does just that.}{does just that.}}\label{engine_autotoc_md142}
...
\DoxyHorRuler{0}
\hypertarget{engine_autotoc_md144}{}\doxysubsubsection{\texorpdfstring{3. Running the Game Loop}{3. Running the Game Loop}}\label{engine_autotoc_md144}
...
\DoxyHorRuler{0}
 \hypertarget{engine_autotoc_md146}{}\doxysubsubsection{\texorpdfstring{4. Behind the Scenes}{4. Behind the Scenes}}\label{engine_autotoc_md146}
...
Each system only cares about the data it needs — that’s the power of ECS.

\DoxyHorRuler{0}
 \hypertarget{engine_autotoc_md148}{}\doxysubsubsection{\texorpdfstring{5. Next Steps}{5. Next Steps}}\label{engine_autotoc_md148}
...
With these, you can start building enemies, bullets, pickups, and much more — all using the same simple ECS principles.
